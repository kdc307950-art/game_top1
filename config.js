...
export const CONFIG = {
  ...
  VINE_MAP_CONFIG: {
    ...
    climbDirection: 'up',
    // 路径锚点（归一化 0–1，**跨越整个世界**）。`up` 时：入口在**世界下方之外**（y > 1）、
    // 出口在**世界上方之外**（y < 0），两端的「出世界」留白正好被平移的极限吃掉，因此
    ...
    tianbianBand: 0.155, // 世界**顶部**云层带的高度比例（19.3 的「第 6 页云层」→ 云带）。
    // 取值约束（由 check-vine-map.mjs 巡检，改坐标就会红）：必须**严格落在**
    // 「最内侧的隐藏关 y（0.15）」与「第 50 关 y（0.16）」之间 —— 云带要盖住 51–53，又不能盖住已解锁的主线末关。

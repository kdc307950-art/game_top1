...
export const CONFIG = {
  ...
  // 藤蔓地图的几何与确定性参数（Step 19.5：**世界坐标 + 视口内纵向平移**，AGENTS.md 2.3 / 5.1；口径见 D047）。
  // 地图是**画布外**的 SVG 层：它不参与 `computeBoardSize`，也不改变既有像素取证。
  // 19.5 的核心口径：**不再分页** —— 整张地图是一块连续的「世界」，视口固定，
  // 平移的是世界自己的 `transform: translateY`（页面本身仍然禁止滚动，5.1 不受影响）。
  // **坐标一律归一化 0–1**：x 相对 `width`、y 相对**世界总高**（0 = 世界顶部、1 = 世界底部）。
  VINE_MAP_CONFIG: {
    seed: 20260922, // 固定种子（用户方案里的 VINE_SEED）：抖动只由 mulberry32(seed) 决定
    width: 360,     // 世界 viewBox 宽（归一化 x 的换算基准）
    height: 640,    // 一个「旧屏」的高度（viewBox 单位）：世界总高 = height × worldHeightRatio
    // 世界总高（= height × 本值）。标定依据：53 个节点 / 27 行 ≈ 每屏 5.9 行 ≈ 19.4 的密度（5.5 行/屏），
    // 既保住「一屏能看清 2 个列位」的读图习惯，也让「向上蔓延」有足够的攀爬长度（约 4.4 屏的平移量）。
    worldHeightRatio: 6.5,
    // 攀爬方向（Step 19.4，用户口径「藤蔓关卡从下往上」）：'up' = 第 1 关在**世界最底**、
    // 关号越大越靠上；路径从**世界外的底部入口**升到**世界外的顶部出口**。
    // 'down' 保留自上而下的行为（坐标生成、巡检与渲染三处共用这一个旋钮）。
    // **22.1（v1.33 / D049）原点翻转**：`y` 一律**自世界底部起算**（0 = 世界最底、1 = 世界最顶），
    // 因此 'up' 时 **y 随关号递增**（第 1 关 ≈ 0.11 → 第 50 关 ≈ 0.89）；屏幕换算 = (1 − y) × 世界总高。
    climbDirection: 'up',
    // 路径延伸点（归一化 0–1，**只有两个**）。
    // **22.2（v1.34 / D049）起曲线不再由「独立锚点 + 抖动」决定**，而是**反向拟合穿过每一个节点**
    // （Catmull-Rom → 三次贝塞尔）：形状完全由 `LEVEL_MAP_POS` 的节点坐标决定。因此 `anchors` 只剩
    // 「两端伸出世界之外」的延伸点 —— 入口在世界下方（y < 0）、出口在世界上方（y > 1），
    // 平移到两端时不会看到藤蔓凭空断掉。x 与首/末节点对齐（0.2 / 0.48），让藤蔓从下方伸进来、向正上方伸出去。
    anchors: [
      { x: 0.2, y: -0.06 },
      { x: 0.48, y: 1.06 }
    ],
    nodeColumns: [0.2, 0.48, 0.76], // 节点 X 列（归一化，≥3 列用于打破「两列对齐」）
    nodesPerRow: 2,    // 每行节点数（行沿世界自下而上推进；2 个/行 → 53 关 = 27 行）
    nodeMarginY: 0.11, // 节点区的上下边距（归一化到**世界**高；同时保证首/末关能被视口居中）
    nodeStaggerY: 0.01, // Y 的错落幅度（归一化）：行内两个节点一高一低，形成节奏（须 < 行距/2）
    ...
    scrollMs: 420,     // 平移/回中的动画时长（ms；替代 19.2 的 pageSlideMs）
    dragThreshold: 8,  // px：手指位移超过它才算「拖拽」，否则仍按「点按节点」处理
    scrollInertia: 0.94, // 松手后每帧的速度衰减（1 = 不衰减；reduced-motion 下不使用惯性）
    navStepRatio: 0.9, // ▲/▼ 一次移动的视口高比例
    // ---- 19.5 视觉（分层视差 + 确定性星光）----
    parallaxFar: 0.25, // 远景（远山）相对世界的**净**位移比例（< 1 = 比世界慢）
    parallaxNear: 1.45, // 近景（星光）相对世界的**净**位移比例（> 1 = 反向/更快）
    particleCount: 150, // 星光点数（确定性；**不使用运行时随机**）
    tianbianBand: 0.155, // 世界**顶部**云层带的厚度比例（19.3 的「第 6 页云层」→ 云带）。
    // 22.1（D049）原点翻转后，`tianbianBand` 仍是「**自世界顶部往下量**的厚度」，云带覆盖 y ∈ [1 − 本值, 1]。
    // 取值约束（由 check-vine-map.mjs 巡检，改坐标就会红）：`1 − tianbianBand` 必须**严格落在**
    // 「第 50 关 y（≈0.89）」与「最内侧的隐藏关 y（≈0.90）」之间 —— 云带要盖住 51–53、又不能盖住已解锁的主线末关。
    backdropBleed: 800, // 天空/地面超出世界上下边缘的出血量（px），保证平移到位时不露白
    // Step 19.4 画风增强（全部程序化，零素材）。只用**已登记**的几何/时长键，
    // 其余纯造型比例（内圈高光、云团偏移、锁形尺寸…）按 D013 留在 vine-map.js 的本地常量。
    arrowSize: 13,     // 当前关指针（箭头）的边长（px）
    arrowOffsetY: 26,  // 指针相对节点圆心的垂直偏移（px，指向下方节点）
    cloudDriftMs: 9000 // 云层缓慢横向漂移周期（ms；prefers-reduced-motion 时关闭）
  },

...
export const CONFIG = {
  ...
  VINE_MAP_CONFIG: {
    ...
    pageSize: 10,   // 每页关卡数（5 页主线 × 10 关 + 第 6 页天边）
    width: 360,     // 地图 viewBox 宽（归一化 x 的换算基准）
    height: 640,    // 地图 viewBox 高（归一化 y 的换算基准）
    // 攀爬方向（Step 19.4，用户口径「藤蔓关卡从下往上」）：'up' = 页内**最下方是第 1 关**、
    // 向上依次推进，路径从**底部入口**升到**顶部出口**（跨屏衔接靠 Y 轴：`入口 y − 1 ≈ 出口 y`）。
    // 'down' 保留 19.2/19.3 的自上而下行为（坐标生成、巡检与渲染三处共用这一个旋钮）。
    climbDirection: 'up',
    // 路径锚点（归一化 0–1）。`up` 时：入口在**底部（页内，y 略小于 1）**、出口在**顶部之外（y 略小于 0）**，
    // 于是「上一页的出口（探进本页上方）↔ 本页的入口（贴在本页底部）」在跨屏处重合
    // —— 衔接条件是 `入口 y − 1 ≈ 出口 y`（0.95 − 1 = −0.05）。
    anchors: [
      { x: 0.3, y: 0.95 },
      { x: 0.75, y: 0.75 },
      { x: 0.2, y: 0.55 },
    ...
    nodeMarginY: 0.08, // 节点区的上下边距（归一化；`up` 时首行在 y = 0.92、末行在 y = 0.08）

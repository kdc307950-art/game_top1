...
**Step 9 落地记录（2026-09-20）**：

- **只借「识别语言」，不借实现**：魔力鸟的外观用「暗色球体 + 六段彩虹环 + 白色中心」表达（对应同类作品里彩虹糖的彩色光芒，5.4 要求可辨），全部 Canvas 程序化绘制（`candy.js` 的 `paintMagicCandy`），未使用其素材。
- **规则口径以本宪法为准**：3.2 只规定「与任意普通色块对调，消除全屏该颜色」，没有规定「被其它特效波及时是否也触发」。本项目取**保守口径**（被波及时不额外触发，`getSpecialAffectedCells(..., 'magic', ...)` 只返回自身），理由与替代方案见 D025 第 5 条。
- **不采纳参考实现的「彩虹糖无颜色」**：candy-crush 的彩虹糖是无色的；本项目 4.1 规定 `color: null` 表示「空格或纯障碍」，故让魔力鸟**保留颜色但被匹配层排除**（4.3.14）。这样渲染、重排与 `cell` 结构都不需要为空色开口子。

**Step 10 落地记录（2026-09-20）**：

- **组合效果只借「有这六种」这一事实，实现完全自定**：`AGENTS.md` 3.3 与附录 A 已把六种组合及其优先级写死，本步因此不需要从参考实现里取任何算法；`references/` 下同类作品的组合代码（尤其是 candy-crush 把组合展开成一棵「效果树」的写法）**未移植** —— 本项目的做法是「就地改造 + 清除集合当激活种子」，只此一处语义（见 D026 第 3 条）。
- **参考实现的「彩色炸弹 + 条纹」等组合在本项目无对应类型**，按 3.3 与「禁止发明规则」不实现；「条纹/包装 + 魔力鸟」也因 3.5 未登记倍数而按已有的 2.5 回落，**不为对齐参考作品而增补数值**。
- **参考实现里常见的「组合触发后立即结算多轮」在本项目由既有级联循环天然覆盖**：`board.resolveCascades` 的循环不区分「组合消除」与「匹配消除」，因此不需要为组合写第二套结算路径。
...
**Step 11 落地记录（2026-09-20）**：

- **只借「层数显示」与「占格/覆层两类障碍」这两个概念，实现完全自定**：`AGENTS.md` 3.4 与附录 A 已把冰块/雪块的层数、受损方式与得分写死；本步没有移植参考实现的障碍物代码（k8scat 的层数显示思路只影响「角标随层数变化」这一点，且用烘焙精灵实现，未复制其素材或代码）。
- **参考项目常见的「障碍物绑在格子上」在本项目被显式采纳并补强**：3.8「重排不得改变障碍物布局」把障碍物钉成**格子属性**，因此交换/下落/补位三处都按位置维护障碍字段 —— 这与把障碍物绑在动物上的写法（会让冰随动物滑走）相反，是本步最重要的取舍（见 D027 第 2 条）。
- **明确不采纳「一次多颗相邻消除各扣一层」**：参考作品里常见按相邻消除数量线性扣层，本项目按 ROADMAP 验收「被波及减 1 层」取**每层最多 −1**（一次组合消掉 5 层冰会变成 25000 分的爆炸性收益）。
- **零素材仍然成立**：冰块用半透明冰蓝覆层 + 白色描边 + 三条避开格心的裂纹；雪块用不透明白色 + 冷色描边 + 底部堆积弧带；层数用深色圆底角标 + 白色数字，全部 Canvas 程序化绘制并烘焙成精灵，未引入任何图片（D009/D020）。
...
**本项目的 50 关设计（2026-09-20，Step 12 开工前定稿）**：不照搬任何参考作品的关卡表，而是按本项目已实现的元素（6 色 + 三种特殊元素 + 冰块/雪块 + 四种目标）自行设计 50 关，并写成 `LEVELS.md`。参考作品在这里只提供两点启发：① 关卡是**数据**而不是代码分支（candy-crush 用 `js/levels/levels.js` 集中声明）；② 难度靠「新机制逐个引入」而不是「同屏堆元素」。本项目的做法是把这两点固化成宪法 3.6 的五条硬指标，并用 `_build/lint-levels.mjs` 自动巡检。

...
## 2026-09-26（第三轮视觉评审的四条 P0 —— 完成，**等用户看图**）

用户对「结算页 + 新版藤蔓地图」两张截图的评价是「整体已经非常接近商业级休闲游戏的完成度」，并给了**四条必须马上修的 P0**（另有 P1 三条与 P2 一条留待下一轮）。**玩法零改动**：只改地图层与结算面板的外观 + 三个探针的判据；口径与两处**根因纠正**记入 `DECISIONS.md` **D053**。

**P0-1「回到当前关」被屏幕边缘裁切**

- **根因不是没留边距，而是挂错了父层**：它原先是 `.vine-viewport` 的子节点，而那个视口只有 `min(100%, 360px)` 宽且 `overflow: hidden`。
- 改为挂到**满屏的 `#map` 层** + 14px 屏边距 + 安全区，做成**带果冻内嵌高光的胶囊**（`linear-gradient` + `inset` 投影 + `max-width: 46vw` + 省略号兜底）。

**P0-2 节点数字（4/5/6）糊进深紫**

- **根因是「整颗节点被压到 `opacity: 0.45`」**（`.vine-node[data-state='locked']` 把 body/label/star 一起压暗）。
- 现在**数字不跟着压暗**：白字 + 深色描边（`paint-order: stroke`）、字重 700 / 15px；锁定节点的数字用 `#ded9f2`、**底盘抬亮到 `#4a3f74`**（用户建议的「更亮的浅色圆盘」），只保留星星的压暗。

**P0-3 藤蔓的「节状感」（打了结的绳子／毛毛虫）**

- **根因不是 `stroke-dasharray`（用户猜的方向），而是 22.2 的「分段渐粗」**：9 段各自 `stroke-linecap: round`，每个接缝都留下一个直径 = 线宽的圆形隆起（垫层 13px 更明显）。
- **`buildVineTaperSegments()` / `.vine-taper` / `.grown` 一并退役**：两层各**一条平滑路径**（垫层 13px + 亮芯 8px），**零 dasharray、零 transition**；「生命感」交给**减量增大**的叶子（`leafSpacing` **70 → 150**、`leafSize` **7 → 9**）。
- 如实说明：这是**对 22.2 一条既有特性的退役**（SVG 的 `stroke` 做不了沿路径的平滑宽度渐变）。若将来仍想要「底部细顶部粗」，**正解是变宽带多边形**（沿路径采样 + 法线偏移生成闭合路径），而不是分段描边 —— 已记入 D053 的替代方案。

**P0-4 结算按钮的粉红与游戏内 UI 体系割裂**

- 旧 `BUTTON_BG = '#ff4fd8'` 当年选它的理由是「不与糖果撞色、便于识别与程序化验证」——**让测试便利决定产品外观**，与 D049 第 2 条批评的反向耦合同源，正式退役。
- 改成**游戏内道具栏的语言**：主按钮**亮金果冻**（`#ffc93b` + 厚底边 `#b8801a` + 42% 白高光）、次按钮**深紫面板**（`#3a3352` + `#1d1830` + 16% 白高光）。全部**纯色几何填充**（每按钮 4 次填充，**零渐变零阴影** —— 结束面板是每帧重绘的，D043/D044 照旧适用）。

**顺手拆掉「探针依赖产品颜色」的耦合（本轮工程上最有价值的一步）**

- `verify-step4`：改用**结构性信号**（棋盘区被 `OVERLAY_DIM` 压暗，用首屏平均亮度做基线 `panelUpIn`）**＋按几何算**按钮中心；不再扫描任何按钮颜色。
- `verify-step12b`：那段「洋红像素聚类 + 点选关」按**规则变更式改写退役**，覆盖**迁移**到 `verify-step24` 的确定性一节（离屏调 `drawGameOver`，用它返回的 `restartRect` / `selectRect` 取样：实测主按钮 **rgb(255,201,59)**、次按钮 **rgb(58,51,82)**，并断言两者**都不再接近洋红**）。
- `verify-step19-2` / `verify-step19-4`：渐粗断言改写成「每层**恰好 1 段**、宽度恒定、**无 dasharray**」。
- **如实登记 P3-M（新）**：「点结束面板的『选关』→ 打开藤蔓地图」这条**端到端路径暂时没有直接探针**（原实现依赖「这一刻面板正好在屏幕上」，而套件前面已多次点过面板/地图，日志判据会拿到**过期**记录 ⇒ 我加了 14 秒重试仍扫不到面板）。下一轮用一个**确定性逼出结束面板**的夹具把它补回来。

**本轮的一次操作事故（如实登记）**：我用 PowerShell 的字符串替换去改探针里的**模板字面量**，反引号被 PowerShell 当成转义符，把三行断言写坏（出现不可见的控制字符，`old_string` 再也匹配不上）；已按**行号**重写修复。教训与 P3-14 同源：**含模板字面量的 JS 文件一律只用编辑工具改**。

**验证**

- `node tests/run-all.js`：**13 文件 / 274 用例 / 3559 断言 / 0 失败**（`tests/vine-map.test.js` 的渐粗用例按新口径改写为「整条藤蔓是**一条**平滑曲线」）。
- `python _build/consistency_check.py`：**全部通过**（v1.39；`config.js` 的叶子数值改动不涉附录 B 键）。
- 浏览器套件**全绿**：`verify-step4`（整局 → 面板 → 点主按钮重开）、`verify-step12b`、`verify-step19-2`、`verify-step19-3`、`verify-step19-4`、`verify-step24`（**19 项**，含新增的「结束面板两个按钮」确定性一节）。
- 截图：`_build/step24-map.png`（地图：胶囊按钮 / 数字对比度 / 平滑藤蔓 / 彩星节点）、`_build/step24-panel.png`（结算页：亮金 + 深灰按钮 + 彩星）。

**未验证（如实）**：① 四条 P0 的**观感**（本机没有视觉模型，请看两张截图；尤其「藤蔓够不够平滑」「数字够不够清楚」「按钮金/灰的比例」）；② 真机手感（胶囊按钮的点击区、数字描边在低分辨率屏上的锐度）；③ P3-M（见上）；④ P1/P2（彩星图标重设计、星星对齐与灰星对比、版面节奏、通关庆祝动效）按用户要求**留到下一轮**。

**下一步**：等你看图验收 → 之后按你的优先级做 P1/P2，或推进 **19.4 的最终验收**。

...
/** HUD 三个信息格的位置（静态图层与文字共用，避免两处各算一遍）。 */
...
export function drawGameOver(ctx, field, overlay) {
  // overlay.reason：'steps'（步数用尽）/ 'stuck'（3.8 约束 4：死局重排超限，判定关卡异常）
  const title = overlay.reason === 'stuck' ? '无可消除组合' : '步数用尽';
  ...
  ctx.textBaseline = 'middle';
  const lines = [
    { text: title, size: 0.085, weight: 700, color: OVERLAY_TITLE_COLOR, at: 0.16 },

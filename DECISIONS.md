...
- 日期：2026-09-26
- 状态：**22.1（坐标口径翻转）落地中**；**22.2（曲线穿过节点）/ 22.3（视觉降噪）待开工**，两者已出原型截图交用户确认（`_build/step22-proto-*.png`）。**玩法零改动**：解锁门槛、天边云层、隐藏关、50 关表、存档、所有逻辑模块一律不动。
...
- 状态：**21.1（HUD 果汁化）与 21.2（道具栏与设置齿轮）均已落地**，两者都在**等用户看截图确认观感**；**21.3（棋子拟人化）待开工**。**玩法零改动**：计分、步数、目标、星级、关卡表、存档、逻辑模块一律不动，只改信息层与外观（21.2 只改画布外的 DOM/CSS，道具的数量/消耗/判定与音效震动的行为、存档键一行未改）。

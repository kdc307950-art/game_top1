...
## D053：第三轮视觉评审的四条 P0（地图按钮裁切 / 数字对比度 / 藤蔓结节 / 结算按钮配色）

- 日期：2026-09-26
- 状态：**已落地**（等用户看图）。**玩法零改动**：只改地图层与结算面板的外观 + 三个探针的判据。
- 背景：用户对「结算页 + 新版藤蔓地图」两张截图给出四条必须马上修的 P0 —— ①「回到当前关」按钮被屏幕边缘裁切；② 节点数字（4/5/6）与深紫底糊在一起；③ 藤蔓像「打了结的绳子／虚线毛毛虫」；④ 结算按钮的粉红与游戏内 UI 体系割裂。同时给了 P1（彩星图标重设计、星星对齐/灰星对比、版面节奏）与 P2（通关庆祝动效）留待下一轮。
- 决定（含两处**根因纠正**）：
  1. **按钮裁切的根因不是「没留边距」，而是挂错了父层**：`buildRecenter()` 原先是 `.vine-viewport` 的子节点，而那个视口只有 `min(100%, 360px)` 宽且 `overflow: hidden` —— 按钮因此贴着窄列边缘、在窄屏上被裁。**改挂到满屏的 `#map` 层** + 14px 屏边距 + 安全区 + 胶囊造型（`max-width: 46vw` + `text-overflow: ellipsis` 兜底）。
  2. **数字看不清的根因是「整颗节点被压到 0.45 透明度」**：`.vine-node[data-state='locked']` 把 body/label/star 一起压暗，解锁态的数字因此糊进深紫。**数字不跟着压暗**：白字 + 深色描边（`paint-order: stroke`）+ 15px/700，锁定节点的数字用 `#ded9f2`、底盘抬亮到 `#4a3f74`（用户建议的「更亮的浅色圆盘」），只保留星星压暗。
  3. **藤蔓结节的根因不是 `stroke-dasharray`（用户猜的方向），而是 22.2 的「分段渐粗」**：9 段各自 `stroke-linecap: round`，每个接缝都会留下一个直径 = 线宽的圆形隆起（垫层 13px 更明显）。**`buildVineTaperSegments()` / `.vine-taper` / `.grown` 一并退役**：两层各**一条平滑路径**，零 dasharray、零 transition；「生命感」交给**减量增大**的叶子（`leafSpacing` 70 → 150、`leafSize` 7 → 9）。**这是对 22.2 一条既有特性的退役**（SVG 的 `stroke` 无法做沿路径的平滑宽度渐变；若将来要「底部细顶部粗」的观感，正解是**变宽带多边形**（沿路径采样 + 法线偏移生成闭合路径），而不是分段描边）。
  4. **结算按钮配色：主题是「游戏内语言」，而不是「探针友好」**：旧的荧光洋红 `#ff4fd8` 当年选它的理由写着「不与糖果撞色、便于识别与程序化验证」—— 那是**让测试便利决定产品外观**（与 D049 第 2 条批评的反向耦合同源）。改为**主按钮亮金果冻**（`#ffc93b` + 厚底边 `#b8801a` + 42% 白高光）/ **次按钮深紫面板**（`#3a3352` + `#1d1830` + 16% 白高光），全部**纯色几何填充**（结束面板是每帧重绘的 ⇒ 不许渐变，D043/D044）。
  5. **顺手拆掉「探针依赖产品颜色」的耦合**：`verify-step4` 改用**结构性信号**（棋盘区被 `OVERLAY_DIM` 压暗，用首屏平均亮度做基线 `panelUpIn`）＋**按几何算**按钮中心；`verify-step12b` 的「颜色聚类 + 点选关」按规则变更式**退役**，覆盖迁移到 `verify-step24` 的确定性一节（离屏调 `drawGameOver`，用它返回的 `restartRect`/`selectRect` 取样：实测主按钮 **rgb(255,201,59)**、次按钮 **rgb(58,51,82)**，且都**不再是洋红**）。地图侧 `verify-step19-2/19-4` 改断言「每层恰好 1 段、宽度恒定、**无 dasharray**」。
- 依据与证据：`tests/run-all.js` **274 用例 / 3559 断言 / 0 失败**（`tests/vine-map.test.js` 的渐粗用例按新口径改写为「整条藤蔓是一条平滑曲线」）；`consistency_check.py` 全通过；浏览器套件 `verify-step4`（整局 → 面板 → 点主按钮重开）、`verify-step12b`、`verify-step19-2/19-3/19-4`、`verify-step24` 全绿；截图 `_build/step24-map.png` / `step24-panel.png`。
- 影响：`hud.js`、`vine-map.js`、`vine-map.css`、`config.js`（leafSpacing/leafSize 数值）、`tests/vine-map.test.js`、`_build/verify-step4/12b/19-2/19-4/24.mjs`、`AGENTS.md`（v1.39）、`ROADMAP.md`（版本）、`PROGRESS.md`、本记录。
- 替代方案：① 保留分段渐粗、只换 `linecap: butt` —— 否决：接头会露出角度缺口，仍不平滑；② 把渐粗改成**变宽带多边形**（真正平滑）—— **暂缓**：这是比本轮 P0 更大的一笔改动（要沿路径采样 + 法线偏移 + 与叶子的采样共用），纳入下一轮候选；③ 保留洋红按钮、只把探针改成几何判据 —— 否决：用户点名「与游戏内 UI 体系割裂」，产品问题不该用探针规避；④ 给按钮加一个「只给探针看」的隐藏色标 —— 否决：那是把测试耦合藏进产品。
- 未验证（如实）：四条 P0 的**观感**（本机没有视觉模型，请看两张截图）；真机手感（胶囊按钮的点击区、数字描边在低分辨率屏上的锐度）；**P3-M（新登记）**：「点结束面板的『选关』→ 打开藤蔓地图」这条**端到端路径**目前没有直接探针（原实现依赖「这一刻面板正好在屏幕上」，而套件前面已多次点过面板/地图、日志判据会拿到过期记录），下一轮补一个「确定性逼出结束面板」的夹具；另有 P1/P2（彩星图标重设计、星星对齐与灰星对比、版面节奏、通关庆祝动效）按用户要求**留到下一轮**。

...
- 日期：2026-09-25
- 状态：**21.1 落地中**（HUD 果汁化）；**21.2（道具栏与设置）/ 21.3（棋子拟人化）待开工**。**玩法零改动**：计分、步数、目标、星级、关卡表、存档、逻辑模块一律不动，只改信息层与外观。
...
- 影响：`config.js`（`HUD_CONFIG` 9 键）、`hud.js`（`goalProgress` / 飘字池 / 心跳 / 进度条绘制）、`render.js`（静态图层卡片烘焙 + 飘字绘制）、`app.js`（`nowMs` 与飘字池接进帧循环）、`tests/hud.test.js`（新）、`AGENTS.md` v1.31、`ROADMAP.md` v1.31、`prompts.md`、`PROGRESS.md`；21.2 动 `index.html` / `styles.css`，21.3 动 `candy.js`。
...
- 未验证（如实）：真机上的帧率与观感（本机无视觉模型，Agent 看不到截图）；`--booster-bar-h` 改成非 100px 时的一批布局断言（21.2 不在本步改动它）。

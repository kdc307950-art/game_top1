...
当一个 Step 已完成、准备进入下一个玩法 Step 时，先执行一次独立的 Bug Audit，不把“功能已经写出来”直接当成“可以继续扩展”。当前 Step 7 已完成，Step 8 必须等本门禁完成后再开始。
...
5. **当前功能边界**：对照 `PROGRESS.md` 与 `DECISIONS.md`，逐项列出已实现、仅有单测、仅代码审查、未验证和明确延期；Step 8-10 的未实现能力不得在验收中预设为可用。

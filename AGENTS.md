...
## 修订说明（v1.38 → v1.39 关键变更）

本版落地**用户视觉评审第三轮的四条 P0**（用户原话：「把 P0 修完，这个界面完全可以拿出去做用户测试」）。**玩法零改动**：计分、步数、目标、星级、关卡表、存档、逻辑模块一律不动，只改**地图层与结算面板的外观**；口径记入 `DECISIONS.md` **D053**。

1. **P0-1「回到当前关」按钮被屏幕边缘裁切**：它原先挂在 `.vine-viewport` 里 —— 那个视口只有 `min(100%, 360px)` 宽且 `overflow: hidden`，按钮因此贴着窄列边缘、在更窄的屏上会被切成半个。**改挂到满屏的 `#map` 层**（`vine-map.js` 的 `host.appendChild(buildRecenter(...))`），CSS 给 **14px 屏边距 + 安全区**，并做成**带果冻内嵌高光的胶囊**（`linear-gradient` + `inset` 投影 + 省略号兜底 `max-width: 46vw`）。
2. **P0-2 节点数字（4/5/6）辨识度太低**：根因是 `.vine-node[data-state='locked']` 把**整颗节点（含数字）压到 `opacity: 0.45`**。现在**数字不跟着压暗**：`.vine-node-label` 改成「白字 + 深色描边（`paint-order: stroke`，字重 700、15px）」，锁定节点的数字用略冷的亮色 `#ded9f2`、底盘抬亮到 `#4a3f74`（用户建议的「更亮的浅色圆盘」），只保留星星的压暗。
3. **P0-3 藤蔓的「节状感」**：根因**不是** `stroke-dasharray`，而是 **22.2 的「分段渐粗」** —— 9 段各自 `stroke-linecap: round`，每个接缝都会留下一个直径 = 线宽的圆形隆起，叠上垫层（13px）就是用户看到的「打了结的绳子／毛毛虫」。**`buildVineTaperSegments()` 与 `.vine-taper` 一并退役**（`vine-map.js` / `vine-map.css` / `tests/vine-map.test.js` 的用例按新口径改写）：两层各**一条平滑路径**（垫层 13px + 亮芯 8px），零 dasharray、零 transition。沿线叶子同时**减量增大**（`leafSpacing` 70 → **150**、`leafSize` 7 → **9**），「生命感」交给叶子而不是线条本身。
4. **P0-4 结算按钮的洋红与游戏内 UI 体系割裂**：`BUTTON_BG = '#ff4fd8'`（当年选它只为「不与糖果撞色、便于探针识别」—— **让测试便利决定产品外观**，正是 D049 第 2 条批评过的反向耦合）退役，改成**游戏内道具栏的语言**：主按钮**亮金果冻**（`#ffc93b` + 厚底边 `#b8801a` + 顶部内嵌高光 42% 白）、次按钮**深紫面板**（`#3a3352` + `#1d1830` + 16% 白高光）。全部是**纯色几何填充**（每按钮 4 次填充，**零渐变零阴影** —— 结束面板是每帧重绘的，D043/D044 照旧适用）。
5. **顺手把「探针依赖产品颜色」这个耦合**一起拆掉：`verify-step4` / `verify-step12b` 原先靠「扫描洋红像素簇」定位结束面板按钮 ⇒ 现在 `verify-step4` 改用**结构性信号**（棋盘区被 `OVERLAY_DIM` 压暗：`panelUpIn` 用首屏平均亮度做基线）**＋按几何算按钮中心**；`verify-step12b` 那段「颜色聚类 + 点选关」按**规则变更式改写退役**（覆盖迁移到 `verify-step24` 的确定性一节，见下），并**如实登记 P3-M**：**「点结束面板的『选关』→ 打开藤蔓地图」这条端到端路径暂时没有直接探针**（原实现依赖「这一刻面板正好在屏幕上」，而套件前面已多次点过面板/地图，日志判据会拿到过期记录），下一轮补一个确定性夹具。
6. **新增的确定性取证**：`_build/verify-step24.mjs` 追加「结束面板的两个按钮」一节 —— 离屏直接调 `drawGameOver`，用它返回的 `restartRect` / `selectRect` **取样两处像素**：实测主按钮 **rgb(255,201,59)**、次按钮 **rgb(58,51,82)**，并断言**两者都不再接近洋红**。地图侧则由 `verify-step19-2` / `verify-step19-4` 按新契约断言「每层**恰好 1 段**、宽度恒定、**无 dasharray**」。
7. **验证**：`tests/run-all.js` **13 文件 / 274 用例 / 3559 断言 / 0 失败**；`python _build/consistency_check.py` 全部通过；浏览器套件 `verify-step4`（整局 → 面板 → 点主按钮重开）、`verify-step12b`、`verify-step19-2`、`verify-step19-3`、`verify-step19-4`、`verify-step24` **全部 PASS**。**如实说明**：本机没有视觉模型，四条 P0 的观感请以 `_build/step24-map.png`（地图：胶囊按钮 / 数字对比度 / 平滑藤蔓）与 `_build/step24-panel.png`（结算页：亮金 + 深灰按钮、彩星）为准；真机手感未测。
8. **本轮的一处操作事故（如实登记）**：我用 PowerShell 的字符串替换去改测试脚本里的模板字面量，**反引号被 PowerShell 当成转义符**，把三行断言写坏了（出现不可见的控制字符）；已按**行号**重写修复。教训与 P3-14 同源：**含模板字面量的 JS 文件一律只用编辑工具改**，不要走 PowerShell 文本管道。
...
## 修订说明（v1.36 → v1.37 关键变更）

本版落地 **Step 23：对局页「满屏竖版」配比** —— 用户问「目前游戏的页面配比符不符合手机游戏的实际配比，《开心消消乐》是多少×多少，要不要改一下」，并在三选一（只做快赢 / **一步到位** / 暂不改）里选了「一步到位」。**玩法零改动**：计分、步数、目标、星级、关卡表、存档、逻辑模块一律不动，只改**画布几何与静态图层**；口径与几何契约变更记入 `DECISIONS.md` **D051**。

1. **画布从「正方形」改成「满屏竖版」**：`computeBoardSize()` **删除** → **`computeCanvasSize() → {w,h}`**（宽 = 视口宽 − 左右安全区，高 = 视口高 − 上下安全区 − `--booster-bar-h`）。实测 iPhone 14 上画布 **390×744（88.2% 屏高）**、**上下留白 0**、画布 + 道具条正好等于屏高；旧版是 358×358、上下各空 193px、**纵向只有 54.3% 被内容占用**。这也让对局页与本来就是 `inset: 0` 满屏层的藤蔓地图**配比一致**。
2. **HUD 改成「跟着屏高走」**：`HUD_RATIO` 0.13 → **0.165**，语义从「HUD 带高 / 棋盘边长」改成「**HUD 带高 / 画布高**」⇒ 实测 **123px = 14.6% 屏高**（旧版 47px = 5.6%，与真实竖屏手游的 15–20% 不在同一档）；`hudCells(canvasW, hudHeight)` 与 `drawHud({ canvasW, hudHeight, … })` 的参数名同步；字号不再写死比例，改为「按带高给目标字号 → `fitSize()` 按宽度线性压到卡片内」，**4–7 位数的分数/最高分绝不溢出卡片**（旧版靠 47px 带高 + 小字号侥幸放得下）。
3. **棋盘区仍是正方形（5.2 不破）且变大**：`boardRect({w,h})` 取 `min(可用宽, 可用高, MAX_BOARD_PX=720)`；`BOARD_MARGIN` 16 → **10** ⇒ 手机上仍是**宽度受限**、棋盘 **311 → 370px**、格边长 **38.9 → 46.25px（+19%）**。
4. **余量变结构、不留空白**：`buildChrome()` 新增**棋盘台面**（从棋盘上沿铺到画布底的浅色圆角台面 + 1px 上沿高光，棋盘坐在上面）与**底部暖光带**；P2 的暖色环境光跨度由「只罩棋盘区」改成**整块画布**。全部仍是**布局期烘焙一次**的几何填充（零素材、零模糊、零每帧渐变）。
5. **几何契约与影响面（本步最大的代价，已同步）**：`boardRect(canvas)` 接受 `{w,h}`；`prepare(canvas, dpr, layout)` 与场景描述里的 **`canvas: {w,h}`**（原 `sizePx`）；`cellAt(canvas, x, y, canvasSize)`；`styles.css` 的 `#board` 兜底尺寸改成满屏竖版（去掉 `aspect-ratio`）；`_build/` 里 **23 个**各自内联旧几何公式的探针（`hudCss = round(sizeCss × 0.13)`、`sideCss = sizeCss − hudCss`、以 `hudCss` 当棋盘顶边 y）统一改成调用 `render.js` 的真实几何，并把棋盘取样点从「HUD 底边」改成「**棋盘区顶边**」；含 `await import` 的页面表达式一律改成 async IIFE（`evaluate` 的 `awaitPromise` 本就是 true）。
6. **验证**：`tests/run-all.js` 全绿；`python _build/consistency_check.py` 全通过（ROADMAP/prompts 的覆盖断言扩到 **Step 0-23**）；新增 `_build/measure-layout.mjs`（6 视口配比表：**上下留白全为 0、滚动 OK、HUD 13.9–14.8% 屏高、格边长 42.5–51.3px**）与 `_build/check-canvas23.mjs`（**12 项 PASS**：画布铺满 / 画布+道具条 = 屏高 / 道具条贴底 / 画布内不出现页面底色 / HUD 占屏 13–18% / 棋盘正方形 / 格边长 ≥42px / 棋盘在 HUD 之下且下方留台面 / 台面上沿有高光 / 棋盘下方有暖光带 / 5.1 不滚动 / 0 异常）；受影响的浏览器套件（`verify-step4`–`verify-step20`、`verify-step19-2/3/4`、`audit-gate-step7`–`11`、`shot-hud-*`、`shot-candy-21`、`shot-booster-21`）全部复跑。
7. **如实说明**：本机**没有视觉模型**，观感请以 `_build/step23-fullpage.png`（780×1688 满屏）与 `_build/step215-board-zoom.png`（棋盘左上 4×4 格放大 4 倍）为准；**真机手感未验证** —— HUD 123px 是否偏高、46px 的格子是否偏大，都需要真机或用户眼睛定夺；平板（`MAX_BOARD_PX` 720 生效）未重新调过配比。

---

## 修订说明（v1.35 → v1.36 关键变更）

本版落地**用户视觉评审第二轮的返工四项（P0-1 / P0-2 / P1 / P1.5 / P2）** —— 用户对 21.3 v2 的判断是「更像『深色面板上的几何表情棋子』，还没形成统一的糖果世界」，并给了优先级；随后追加一条硬伤「道具栏文字换行太出戏，必须确保单行显示」。**玩法零改动**：计分、步数、目标、星级、关卡表、存档、逻辑模块一律不动；口径记入 `DECISIONS.md` **D050**。

1. **P0-1 特殊糖果的识别度（发光线 + 高对比条纹）**：`candy.js` 新增 `paintSpecialAura()` —— 条纹 / 包装 / 魔力鸟外围套**两层几何描边**（1.10r 淡、1.04r 亮），**普通糖果不套**（保持素，让特殊棋子跳出来，探针实测轮廓面积比 1.31–2.29 倍）。条纹白纹下先铺一条**底色自己的深色版**（`mixHex(base, '#000000', 0.62)`）—— ⚠️ 这条是**硬约束**：中性黑会把红糖果的格心色相从 358° 拉到 330°（偏 28°），而 `verify-step12b` 的「按色相识别 64 格」与 `verify-step14` 的收集物配色**就是靠格心取样**的。实测条纹亮度极差全部 ≥ 120（黄糖果 141，用户点名的「浅色最吃亏」）。途中还抓到 **miter 尖角缺陷**（三角形锐角处的接头会戳到 1.58r、被精灵边界裁掉）⇒ 改 `lineJoin/lineCap = 'round'`。
2. **P0-2 步数 ≤5 的红色警告 + 心跳**：`HUD_WARN_COLOR` 由品牌金 `#ffd93b` 改**红 `#ff4d5e`**（金色从此只用于正向反馈 —— 达标闪烁与进度条），红色只用于预警；新增 `drawWarnFrame()`（红框 + 一圈随心跳向外扩散的描边，纯几何 1–2 次描边，零模糊零每帧渐变）；心跳与字号缩放**同源**（同一个 `heartbeatScale` phase）。新增探针 `_build/shot-hud-warn.mjs`（**10 项 PASS**：步数 9 → 0 红像素、阈值 5 起报红、红框只套步数那一格、时间关同一条路径、心跳纯函数 1.000↔1.140）。
3. **P1 道具栏统一成 HUD 语言 + 标签必须单行**：底色由荧光粉改**深紫面板**（`#241f3a` + 半透明白顶光），颜色只出现在 **26×26 的果冻图标**上（刷新蓝 206° / 加步绿 137° / 木锤琥珀 35°，各带顶部径向高光与 `inset 0 -3px` 厚底边）；**用光（`disabled`）** 更暗 + 果冻退灰。**用户点名的硬伤**：标签统一成**两个字**（刷新 / 加步 / 木锤；`aria-label` 保留完整名称「加五步 / 小木锤」）+ `white-space: nowrap`，探针在 **390px 与 360px** 两种视口下断言「`getClientRects().length === 1`（不折行）、无溢出、三键等宽」。
4. **P1.5 三处普通糖果细节**：① 红圆的环形内嵌图案 → **径向渐变暗核**（沿半径最大亮度台阶 6 ≤ 26，中心仍比 0.7r 处暗）；② 多边形/星形按 `shape.round` 用 `arcTo` **倒角** —— 新增 `filletRadius()` **逐角夹紧**（`arcTo` 的切点距离是 `R / tan(ψ/2)`，而**五星的边长只有 0.73r**，统一 0.13r 会让相邻圆角重叠自交），同一条规则也给了三角/五星/六边形；③ 橙方用**逐形状 `rim` / `lift`**（0.32 / 0.035）让暗边更轻（下缘亮度 **141 vs 绿菱 100**，不再「被框死」）。探针口径同步升级：从「量顶点那一行的长度」（只有 0–1px 覆盖率、全看抗锯齿）改成「**从顶点往下到宽度首达 6px 的行数**」。
5. **P2 棋盘暖色环境光**：`render.js` 的静态图层叠一层**竖直渐变 + 加色混合**的暖光（顶 0.085 → 底 0.03），**只在布局期烘焙一次**、零模糊滤镜（D043/D044）。取证在渲染出来的画布上取「格子间隙的十字交叉处」（离最近糖果圆心 0.71 格 ⇒ 连发光线都够不到）：格线 R−B 由 **−22 → 顶部 −9 / 底部 −17**、最暖处亮度 **48**（暗盘没被洗白）。
6. **验证**：`tests/run-all.js` **13 文件 / 272 用例 / 3396 断言 / 0 失败**；`consistency_check.py` 全通过；`shot-candy-21` **33 项**、`shot-booster-21` **46 项**、`shot-hud-warn` **10 项** 全部 PASS，`shot-hud-21` 实测**单帧 85 次绘制调用 ≤ 200、每帧新建渐变 0**（本轮把**径向渐变**也计入该计数器，此前只统计线性渐变 —— 那是探针的漏洞，不是产品问题）；受影响的 `verify-step10/11/12/12b/13/14/17/20` 复跑全绿。**如实说明**：本机没有视觉模型，观感请以 `_build/step213-sheet.png`（30 张图鉴）、`step213-board.png`（真机棋盘）、`step21-booster-closed.png` / `step21-booster-settings.png`（道具栏）为准；心跳外圈的扩散幅度在真机上只有约 0.5px（主要信号是字号 18→21px 与红框）；真机帧率与手感未测。

---

## 修订说明（v1.34 → v1.35 关键变更）

本版落地 **Step 21 的最后一片：21.3 棋子拟人化与质感重烘焙**（**Step 21 三步至此全部落地**）。**玩法零改动**：计分、步数、目标、星级、关卡表、存档、逻辑模块一律不动，只改 `candy.js` 里**布局期烘焙**的精灵画法；口径仍记在 `DECISIONS.md` **D048**。

1. **拟人化（6 色 × 5 态 = 30 张精灵全部有脸）**：新增 `paintFace()` —— 两只小眼睛（眼珠 + 一点白色反光）+ 两片腮红。**眼睛用的是「糖果自己底色的深色版」**（`mixHex(base, …, EYE_MIX)`）而不是纯黑，且**表情全部避开格心**（`|dx| < 0.1r`、`|dy| < 0.12r` 是空区）—— 这两条是**为既有取证留的路**：`verify-step12b` 的「按色相识别 64 格」与 `verify-step14` 的收集物配色判定都靠格心/近格心取样，实测它们的色相偏移 **≤ 2°**、**全部复跑通过**。
2. **表情画在最后一层（顺序是有讲究的）**：`paintCandy` 增加 `withFace` 开关；**条纹与包装**先画各自的覆盖物（3 条白纹 + 方向箭头 / 四个包装结），**脸最后画**，否则会被盖住。魔力鸟的脸画在**白色中心**上，并**裁剪在核心圆内**（否则腮红会溢出到彩虹环上、左右踩到不同颜色的环段而不对称）；`MAGIC_CORE_RATIO` 因此由 **0.24 → 0.4**（原来的白核放不下脸）。
3. **质感靠几何，不靠滤镜（15 节红线）**：新增**预烘焙投影** —— 形状的深色剪影往右下偏移（`+0.05r, +0.11r`）画在最底层，做出「厚投影」；再加**第二点小高光**做出「糖壳反光」。全部是**几何填充**，`candy.js` 里**没有**任何 `shadow*` 模糊类 API（`verify-step17` 的红线 grep 复跑通过），也没有每帧新建渐变（都在烘焙期）。
4. **顺手修掉一个潜伏缺陷（本轮最有价值的产出）**：`shapePath` 里的 `squash` 是**纵向拉伸**（`(1 + squash)`），而黄糖果的三角形原本是 `outer 1.12 + squash 0.62` ⇒ 纵向半高 **1.81r**，远超精灵的 0.5 格（1.39r）⇒ **三角形的顶部顶点一直被精灵边界裁掉**（实测：最外 1px 有 13 个不透明像素、`minY = 0`，即黄棋子的尖被削平）。改为 `outer 0.88 + squash 0.45` ⇒ 纵向半高 1.276r，加上投影偏移仍在 1.389r 内 —— 修复后**30 张精灵的最外 1px 全部透明**。这是**修 bug**，不是改规则（三角形仍是三角形，色盲友好的形状↔颜色映射不变）。
5. **腮红颜色的取舍（实测驱动）**：最初的深粉（`rgba(255,108,132,0.3)`）在**红糖果**上几乎不可辨（红压红，与底色的 L1 距离只有 23）⇒ 改为偏**浅**的粉（`rgba(255,186,202,0.5)`），6 色上的 L1 距离变成 **102–111**（可测、也真的看得见）。
6. **验证**：`tests/run-all.js` **13 文件 / 272 用例 / 3396 断言 / 0 失败**；`python _build/consistency_check.py` 全部通过（v1.35）；新增 `_build/shot-candy-21.mjs`（**11 项 PASS**）—— 直接读**烘焙出来的像素**：30 张精灵**每张**都有两只眼睛、18 张可断言的精灵腮红在位、腮红左右对称（**排除五星：它本身不是竖直镜像对称的**）、格心色相偏移 ≤ 2°、最外 1px 全透明、普通糖果右下象限有 ≥ 200 个半透明像素（几何投影）、图集仍是 6×5=30；两张截图（30 张放大图鉴 + 真实棋盘）。受影响的既有套件 **`verify-step10` / `11` / `12` / `12b` / `13` / `14` / `17` / `20`（45/45）** 全部复跑通过。**如实说明**：本机没有视觉模型，Agent 看不到截图，观感请以 `_build/step213-sheet.png`（图鉴）与 `_build/step213-board.png`（真机尺寸棋盘）为准；**魔力鸟的脸是 30 张里最小的**（白核 0.4r × 1.7 ≈ 0.68r），真机上可能仍偏小。
...
- `render.js`：棋盘层绘制与几何计算（画布尺寸与 DPR、棋盘布局、静态图层烘焙、每帧贴图与几何命中）。只接收「场景描述」对象，不读游戏状态、不绑定事件、不碰存档；单向依赖 `hud.js` 取布局常量与卡片材质（`hudCardStyle()`）、`candy.js` 取糖果精灵。**v1.31（D048）**：HUD 卡片（投影 / 卡片渐变 / 顶部高光）在 `buildChrome()` 里**布局期烘焙一次**，运行期仍只有 `drawImage` 与文本绘制；`draw()` 追加「连击文案（复用 `drawBanner` 通道）→ 分数飘字」两道，并把 `scene.nowMs` 透传给 `drawHud`（探针实测单帧 85 次绘制调用、每帧 0 新建渐变）。**v1.37（D051）几何契约变更**：`computeBoardSize()` 删除，改为 **`computeCanvasSize() → {w,h}`**（满屏竖版画布）；**`boardRect(canvas)` 接受 `{w,h}`** 并给出 `{ hudHeight, field: {x,y,side} }`（顶部满宽 HUD 带 + 正方形棋盘区 + 底部台面）；`prepare(canvas, dpr, layout)` 与场景描述里的 **`canvas: {w,h}`**（原 `sizePx`）、`cellAt(canvas, x, y, canvasSize)` 同步；`buildChrome()` 另烘焙**棋盘台面**（浅色圆角台面 + 1px 上沿高光）与**底部暖光带**，P2 的暖色环境光跨度改为**整块画布**。
...
- Canvas 按 `devicePixelRatio` 适配；**画布是满屏竖版**（宽 = 视口宽 − 左右安全区，高 = 视口高 − 上下安全区 − `--booster-bar-h`，见 v1.37），**棋盘区保持正方形**（`boardRect` 取「可用宽 / 可用高 / 720」的较小者；手机上宽度受限）。
...
| v1.35 | 2026-09-26 | Agent（用户批准：Step 21 三片全做） | **Step 21.3 棋子拟人化与质感重烘焙**（玩法零改动，只改 `candy.js` 的布局期烘焙）：新增 `paintFace()` —— 30 张精灵（6 色 × 5 态）全部有「两只小眼睛 + 一点反光 + 两片腮红」；眼睛用**底色的深色版**且表情**避开格心**（`verify-step12b` 的按色相识别实测偏移 ≤ 2°）；脸在条纹/包装/魔力鸟上**最后画**（否则被覆盖），魔力鸟的脸**裁剪在白色核心内**且 `MAGIC_CORE_RATIO` 0.24 → 0.4；新增**预烘焙几何投影**（右下偏移剪影）与第二点高光，全程零 `shadow*` 模糊类 API；**顺手修掉潜伏缺陷**：三角形 `squash` 是纵向拉伸（`1+squash`），原 `outer 1.12 + squash 0.62` 让纵向半高达 1.81r、**顶部顶点一直被精灵边界裁掉**（实测最外 1px 有 13 个不透明像素），改 `outer 0.88 + squash 0.45` 后 30 张精灵最外 1px 全透明。**验证**：272 用例 / 3396 断言 / 0 失败、`consistency` 全通过、新增 `_build/shot-candy-21.mjs` **11 项 PASS**（读烘焙像素：眼睛/腮红/对称/格心色相/不越界/几何投影/图集 6×5=30）、受影响套件 `verify-step10/11/12/12b/13/14/17/20`（45/45）全绿 | 2.3、11、`candy.js`、`AGENTS.md`、`ROADMAP.md`、`DECISIONS.md` D048、`PROGRESS.md` |

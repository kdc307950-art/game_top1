...
/
  ...
  candy.js         # 糖果外观与精灵烘焙：形状路径、配色、内嵌图案、条纹特效（D020）
...
- `candy.js`：糖果外观与精灵烘焙（形状路径、配色、内嵌图案、条纹特效及其方向箭头）。只接收坐标、颜色与形状参数，不认识棋盘状态、不读游戏状态、不绑定事件、不碰存档；依赖方向为 `render.js → candy.js` 单向，不得反向依赖。
